...
// Comment the line below to not have phantom refs (you need to uncomment the line above)
        ...
        Options.v().set_output_format(Options.output_format_jimple);//输出按照Jimple模式
    ...
    public static void main(String[] args) {
        ...
        for (Unit u : body.getUnits()) {
            ...
            //报告invoke的情况并返回是否是int area()的签名同时由Circle类声明
...
//            Visualizer.v().addCallGraph(callGraph);
    ...
    private static void reportTrapInfo(JimpleBody body){
        System.out.println("=====================Trap Info Start=========================");
        Chain<Trap> trapChain = body.getTraps();
        for(Trap trap:trapChain){
            //try里面的第一条语句
            System.out.println(String.format("trap first unit: %s",trap.getBeginUnit().toString()));
            //try里面的最后一条语句
            System.out.println(String.format("trap last unit: %s",trap.getEndUnit().toString()));
            //返回处理异常的代码单元
            System.out.println(String.format("trap's handler: %s",trap.getHandlerUnit().toString()));
            // exception类型
            System.out.println(String.format("trap's exception: %s",trap.getException().toString()));

            //本身Trap是支持toSring的
            //System.out.println(trap);
        }
        System.out.println("=====================Trap Info End=========================");
    }
    private static void reportLocalInfo(JimpleBody body) {
        System.out.println("=====================Local Info Start=========================");
        ...
        for(Local local:localChain){
            System.out.println(String.format("local name %s ---> local type %s",local.getName(),local.getType()));
        }
        Local thisLocal = body.getThisLocal();
        Type thisType = thisLocal.getType();
        Local paramLocal = body.getParameterLocal(0);
        System.out.println("=====================Local Info Stop=========================");
    }
    ...
    private static void modifyBody(JimpleBody body, Stmt stmt) {
        ...
        stmt.apply(new AbstractStmtSwitch() {
            ...
            public void caseIfStmt(IfStmt stmt) {
                ...
                System.out.println(String.format("    (After change) if condition '%s' is false goes to stmt '%s'", stmt.getCondition(), stmt.getTarget()));
            }
        });
        // 用于验证body的正确性
        //body.validate();
